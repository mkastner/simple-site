...
async function buildHashedPageAssets(page) {
  ...
  const justMoveRegex = /(\/.*\.(gif|jpg|jpeg|png|ico))/gim;
  ...
  const matchedJustCopyUris = [...page.matchAll(justMoveRegex)].map((match) => {
    return match[1];
  });

  //matchedJustCopyUris.push('/fonts');

  log.info('matchedCssUris', matchedCssUris);
  log.info('matchedJsUris', matchedJsUris);
  log.info('matchedSvgUris', matchedSvgUris);
  ...
  try {
    ...
    for (let i = 0, l = names.length; i < l; i++) {
      ...
      if (stat.isFile() && childPath.match(/\.md$/)) {
        ...
        //if (basePath !== 'index') {
        const pages = await buildPathContent(srcRootDir, basePath);
