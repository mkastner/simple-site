...
const uriHookPath = Path.join(cwd, 'site/lib/hooks', 'sitemap-uri.js');
...
module.exports = async function Sitemap(newOptions = {}) {
  ...
  traverseDirectory.fileGlobPatterns = ['*.html'];

  log.info(
    'traverseDirectory.fileGlobPatterns',
    traverseDirectory.fileGlobPatterns
  );
  ...
  await traverseDirectory.event.addAsyncListener(
    ...
    async ({ file, initialDirectory }) => {
      ...
      const hookedUriPath = options.hook(uriPath);
      log.debug('hookedUriPath', hookedUriPath);

      let loc = hookedUriPath;
