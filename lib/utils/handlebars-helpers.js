module.exports = {
  ...
  submenuFor(menuItems, pattern, reqPath, options) {
    for (let i = 0, l = menuItems.length; i < l; i++) {
      const menuItem = Object.assign({}, menuItems[i]); 
      if (menuItem.url.indexOf(pattern) === 0 && menuItem.menu) {
        return options.fn(menuItem); 
      }
    }
  }, 
  menu(menuItems, reqPath, options) {
    if (!menuItems) return 'No menu items passed to menu helper';
    let result = ''; 
    for (let i = 0, l = menuItems.length; i < l; i++) {
      const menuItem = Object.assign({}, menuItems[i]); 
