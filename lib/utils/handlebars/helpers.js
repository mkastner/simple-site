...
module.exports = function Helpers() {
  ...
  return {
    ...
    block(blockName) {
      const ev = blockEvents[blockName];
    ...
    contentFor(blockName, options) {
      blockEvents[blockName] = () => options.fn();
    },
    textile(s) {
      if (!s) return '';
      return textile(s);
    ...
    includeInline(uriPath) {
      const srcPath = Path.join(DirLocations.src.absolute, uriPath);
      if (!fileExistsAtPath(srcPath))
        return `can't find file at path ${uriPath}`;
