...
module.exports = function buildAsset(urlOrPath) {
  ...
  return {
    async writeCss(modifyCss = async (css) => css) {
      ...
      const scssData = fs.readFileSync(
        Path.join(sourceDir, `${stem}.scss`),
        'utf8'
      );

      const modifiedScssData = await modifyCss(scssData);

      if (!modifiedScssData)
        throw new Error('modifyCss() must at least return original css');

      const result = sass.renderSync({
        //file: Path.join(sourceDir, `${stem}.scss`),
        data: modifiedScssData || scssData,
        sourceMap: true,
        outFile: publicPath,
      });
