...
module.exports = async function buildPathContent(viewsDir, reqPath) {
  ...
  if (!loadedTemplate) error += `Could not find template file for path ${reqPath}`;  
  ...
  const template = compiledTemplate({htmlText, menu, meta, reqPath});
  const page = compiledLayout({body: template, menu, meta, reqPath});
