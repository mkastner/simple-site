...
async function load() {
  try {
    ...
    const apiSegmentsResult = await axios({
      url: `https://www.fmh.de/api/topics/children/${topicIds.join('-')}/segment`,
    });

    const orderedSegments = apiSegmentsResult.data.sort((a, b) => a.position - b.position );
      ...
      eachVariant: function eachVariant(clos) {
        ...
        for (let i = 0, l = topics.length; i < l; i++) {
          ...
          clos({
            ...
            loaderData: { topic, segments: apiSegmentsResult.data.filter(segment => segment.parent_id === topic.id) },
            pathSection: preferredTopicUrl(topic),
            length: l,
          });
        }
      },
